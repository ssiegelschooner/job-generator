...
if (defined('WP_CLI') && WP_CLI) {
    ...
    // use like wp generate_job_posts --count=15
    WP_CLI::add_command('generate_job_posts', function($args, $assoc_args) {
        ...
        for ($i = 1; $i <= $num_posts; $i++) {
            ...
            // Create a new post
            $new_post = [
                'post_title'    => "Job Position $i in $category",
                'post_content'  => "This is a job posting for $category.",
                'post_status'   => 'publish',
                'post_author'   => 1,
            ...
            WP_CLI::success("Created post ID $post_id with title 'Job Position $i in $category'");
        }
